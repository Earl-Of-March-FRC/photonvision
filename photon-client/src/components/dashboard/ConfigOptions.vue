...
const tabGroups = computed<ConfigOption[][]>(() => {
  ...
  const isAprilTagAlgae = useCameraSettingsStore().currentWebsocketPipelineType === WebsocketPipelineType.AprilTagAlgae;
    ...
    .map((tabGroup) =>
      tabGroup.filter(
        (tabConfig) =>
          ...
          !(!(isAprilTag || isAprilTagAlgae) && tabConfig.tabName === "AprilTag") && //Filter out apriltag unless we actually are doing AprilTags
