...
public class AlgaePipeline extends CVPipeline<CVPipelineResult, AlgaePipelineSettings> {
        ...
        protected void setPipeParamsImpl() {
                ...
                HSVPipe.HSVParams hsvParams = new HSVPipe.HSVParams(
                                settings.hsvHue, settings.hsvSaturation, settings.hsvValue, settings.hueInverted);
                hsvPipe.setParams(hsvParams);

                EdgeMaskPipe.MaskParams maskParams = new EdgeMaskPipe.MaskParams(settings.erosion,
                                settings.initialDilation, settings.edgeThresholds.getFirst(),
                                settings.edgeThresholds.getSecond(), settings.edgeDilation, settings.finalDilation);
                ...
                AlgaeDetectionPipe.AlgaeDetectionParams algaeDetectionParams = new AlgaeDetectionPipe.AlgaeDetectionParams(
                                160.0, settings.contourArea.getFirst(), settings.contourArea.getSecond(),
                                frameStaticProperties, settings.circularity.getFirst(),
                                settings.circularity.getSecond(), frameStaticProperties.cameraCalibration,
                                settings.padding);
                algaeDetectionPipe.setParams(algaeDetectionParams);

                Collect2dTargetsPipe.Collect2dTargetsParams collect2dTargetsParams = new Collect2dTargetsPipe.Collect2dTargetsParams(
                ...
                Draw2dTargetsPipe.Draw2dTargetsParams draw2DTargetsParams = new Draw2dTargetsPipe.Draw2dTargetsParams(
                ...
                Draw2dCrosshairPipe.Draw2dCrosshairParams draw2dCrosshairParams = new Draw2dCrosshairPipe.Draw2dCrosshairParams(
                                settings.outputShouldDraw,
                                settings.offsetRobotOffsetMode,
                                settings.offsetSinglePoint,
                                dualOffsetValues,
                ...
                var draw3dTargetsParams = new Draw3dTargetsPipe.Draw3dContoursParams(
                                settings.outputShouldDraw,
        ...
        protected CVPipelineResult process(Frame frame, AlgaePipelineSettings settings) {
                ...
                List<CVShape> algaeShapes = algaeResults.stream().map(AlgaeResult::getShape)
                                .collect(Collectors.toList());

                List<PotentialTarget> potentialTargets = algaeShapes.stream()
                                .map(
                                                shape -> {
                ...
                if (settings.solvePNPEnabled) {
                        for (int i = 0; i < targetList.size(); i++) {
                                targetList.get(i).setBestCameraToTarget3d(
                                                algaeResults.get(i).getCameraToAlgaeTransform());
                                targetList.get(i).setAltCameraToTarget3d(new Transform3d());
                        }
                }

                frame.processedImage.copyFrom(unPadResult.output);
